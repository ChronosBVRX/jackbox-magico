...
def add_points(room_id: int, player_name: str, points: int):
    if points == 0:
        return
    player = supabase.table("players").select("id, score").eq("room_id", room_id).eq("name", player_name).execute()
    if player.data:
        supabase.table("players").update({"score": player.data[0]["score"] + points}).eq("id", player.data[0]["id"]).execute()
...
async def start_game(room_code: str, game_id: str):
    ...
    meta = GAME_CATALOG[game_id]
    new_state["game_id"] = game_id
    new_state["game_name"] = meta["name"]
    new_state["about"] = meta["about"]
    new_state["how_to_play"] = meta["how_to_play"]
    new_state["rules"] = meta["rules"]

    supabase.table("rooms").update({"status": "playing", "game_state": new_state}).eq("room_code", room_code.upper()).execute()
    return {"message": f"{GAME_CATALOG[game_id]['name']} iniciado", "game_id": game_id}
...
async def create_room():
    ...
    supabase.table("rooms").insert({"room_code": code, "status": "lobby", "game_state": {"phase": "lobby"}}).execute()
...
async def submit_answer(info: AnswerInfo):
    ...
    if phase == 'duelo':
        ...
        wins = ((info.answer == "Protección" and enemy == "Contraataque") or
                (info.answer == "Contraataque" and enemy == "Esquivar") or
                (info.answer == "Esquivar" and enemy == "Protección"))
        if wins:
...
async def return_lobby(room_code: str):
    supabase.table("rooms").update({"status": "lobby", "game_state": {"phase": "lobby"}}).eq("room_code", room_code.upper()).execute()
