"""Copa de las Casas: Pregunta Final.

Lógica pura para una ronda final con apuesta secreta por casa.
La arquitectura actual usa polling REST; los eventos devueltos son fáciles de emitir
por Socket.IO si después se agrega un servidor en tiempo real.
"""
...
FINAL_POOL = [
    {
        "final_type": "sabiduria",
        "final_label": "Final de Sabiduría",
        ...
        "correct": "El diario de Tom Riddle",
        "explanation": "En la Cámara Secreta, Harry usa el colmillo del basilisco para destruir el diario de Tom Riddle.",
        "narrator": "Final de Sabiduría: aquí no gana quien grita más fuerte, sino quien sí puso atención entre papas y refresco.",
    },
    {
        "final_type": "valor",
        "final_label": "Final de Valor",
        "question": "Respuesta en 5 segundos: ¿qué hechizo se usa para desarmar al oponente?",
        "options": ["Expelliarmus", "Lumos", "Accio", "Alohomora"],
        "correct": "Expelliarmus",
        "explanation": "Expelliarmus es el encantamiento desarmador más reconocido en los duelos.",
        "narrator": "Final de Valor: cinco segundos. Ni el Ministerio responde tan rápido, pero ustedes sí tienen que hacerlo.",
    },
    {
        "final_type": "lealtad",
        "final_label": "Final de Lealtad",
        "question": "Sin hablar entre ustedes: ¿qué cualidad representa mejor a Hufflepuff?",
        "options": ["Lealtad", "Ambición", "Ingenio", "Temeridad"],
        "correct": "Lealtad",
        "explanation": "Hufflepuff se asocia con la lealtad, el trabajo honesto y la constancia.",
        "narrator": "Final de Lealtad: si los dos integrantes no coinciden, el castillo sospechará desorganización emocional.",
        "conflict_mode": "mark_conflict",
    },
    {
        "final_type": "astucia",
        "final_label": "Final de Astucia",
        "question": "Pregunta con trampa: si una capa de invisibilidad te vuelve invisible, ¿qué NO oculta necesariamente?",
        "options": ["El ruido que haces", "Tu sombra mágica", "El color de tus calcetas", "El tamaño de tu varita"],
        "correct": "El ruido que haces",
        "explanation": "Ser invisible no significa ser silencioso. La astucia también consiste en no caminar como hipogrifo con botas.",
        "narrator": "Final de Astucia: cuidado, la respuesta correcta está escondida como excusa de alumno que no estudió.",
    },
    {
        "final_type": "memoria",
        "final_label": "Final de Memoria",
        "question": "Recuerda la secuencia mágica mostrada: ¿cuál fue el orden correcto?",
        "sequence": ["🪄 Varita", "📖 Diario", "🏆 Copa", "🦉 Lechuza"],
        "options": [
            "Varita → Diario → Copa → Lechuza",
            "Diario → Varita → Lechuza → Copa",
            "Copa → Varita → Diario → Lechuza",
            "Lechuza → Copa → Diario → Varita",
        ],
        "correct": "Varita → Diario → Copa → Lechuza",
        "explanation": "La secuencia correcta era Varita, Diario, Copa y Lechuza.",
        "narrator": "Final de Memoria: el castillo mostrará una secuencia. No parpadeen como si fuera final de telenovela.",
    },
    {
        "final_type": "escena",
        "final_label": "Pregunta de Escena",
        ...
        "narrator": "Pregunta de escena: si no recuerdan esto, mínimo respondan con dignidad teatral.",
    },
]
...
def _public_wager_status(private_wagers: dict) -> dict:
    return {
        ...
        for house, entry in (private_wagers or {}).items()
    }
...
def build_state(previous_state: Optional[dict] = None) -> dict:
    ...
    options = _shuffle_options(item.get("options", []))
    ...
    return {
        ...
        "question_payload": {
            ...
            "narrator": item.get("narrator") or random.choice(NARRATOR_OPENING),
            "duration_seconds": duration,
        },
        "narrator": random.choice(NARRATOR_OPENING),
        "intro_lines": NARRATOR_OPENING,
        "wager_options": [
            {"label": "0 puntos", "value": 0, "all_in": False, "tone": "safe"},
            {"label": "100 puntos", "value": 100, "all_in": False, "tone": "low"},
            {"label": "200 puntos", "value": 200, "all_in": False, "tone": "medium"},
            {"label": "300 puntos", "value": 300, "all_in": False, "tone": "high"},
            {"label": "Todo o nada", "value": ALL_IN_VALUE, "all_in": True, "tone": "legendary"},
        ],
        ...
        "answer_conflicts": {},
        "settings": {
            "conflict_mode": item.get("conflict_mode", DEFAULT_CONFLICT_MODE),
            "cap_wager_to_house_score": True,
            "missing_wager_defaults_to_zero": True,
            "all_in_bonus": ALL_IN_BONUS,
        },
        "visual": {
            "scene": "great_hall_final_ceremony",
            "background": "Gran Comedor, copa brillante, velas flotantes, escudos de casas, luz dorada cinematográfica",
            "accent": "gold_epic_finale",
        },
        "sound_cue": "reveal",
    }
...
def house_scores(players: List[dict]) -> Dict[str, int]:
    ...
    for player in players or []:
        ...
        if house not in scores:
            scores[house] = 0
        scores[house] += int(player.get("score") or 0)
    return scores
...
def resolve_wager_amount(value: Any, house_score: int, cap_to_score: bool = True) -> dict:
    ...
    return {
        "raw": int(selected),
        "amount": int(capped_amount),
        "all_in": False,
        "label": f"{int(selected)} puntos",
        "capped": int(capped_amount) != int(selected),
    }
...
def submit_wager(state: dict, player_name: str, player_house: str, wager: Any, players: List[dict]) -> dict:
    ...
    if player_house in wagers_by_house:
        previous = wagers_by_house[player_house]
        return {
            "state": state,
            "accepted": True,
            "already_submitted": True,
            "message": f"{player_house} ya selló su apuesta. La primera decisión se conserva.",
            "wager": {"sealed": True, "label": previous.get("label")},
        }
    ...
    entry = {
        **wager_info,
        ...
        "house_score_at_bet": int(scores.get(player_house, 0)),
    }
    ...
    state["narrator"] = random.choice([
        "Apuesta sellada. El castillo guarda secretos mejor que un grupo de WhatsApp familiar.",
        "La casa ha apostado. Ahora toca fingir tranquilidad.",
        "Apuesta recibida. La Copa acaba de ponerse interesante.",
    ])
    ...
    for house in active_houses(players):
        if house not in wagers_by_house:
            wagers_by_house[house] = {
                ...
                "submitted_by": None,
                "submitted_at": time.time(),
                "house_score_at_bet": int(scores.get(house, 0)),
                "auto": True,
            }
...
def submit_house_answer(state: dict, player_name: str, player_house: str, answer: Any, client_elapsed_ms: Optional[int] = None) -> dict:
    ...
    conflict_mode = settings.get("conflict_mode", DEFAULT_CONFLICT_MODE)
    ...
    if player_house not in house_answers:
        house_answers[player_house] = {
            "house": player_house,
            "answer": answer_text,
            "submitted_by": player_name,
            "submitted_at": time.time(),
            "elapsed_seconds": elapsed_seconds,
            "responses": {player_name: answer_text},
        }
        ...
        if normalize_text(current.get("answer")) != normalize_text(answer_text):
            conflicts[player_house] = {
                "house": player_house,
                "first_answer": current.get("answer"),
                "new_answer": answer_text,
                "first_by": current.get("submitted_by"),
                "conflict_by": player_name,
                "mode": conflict_mode,
            }
...
def reveal_results(state: dict, players: List[dict]) -> dict:
    ...
    for house in active_houses(players):
        ...
        delta = amount if is_correct else -amount
        ...
        house_results.append({
            ...
            "answer": answer_text,
            "correct": is_correct,
            "forced_wrong_by_conflict": forced_wrong,
            "conflict": conflict,
    ...
    result = {
        "title": "Resultado de la Pregunta Final",
        "final_label": state.get("final_label"),
        "final_type": state.get("final_type"),
        "question": state.get("question"),
        "correct_answer": correct_answer,
        "explanation": (state.get("question_payload") or {}).get("explanation"),
        "houses": house_results,
        "largest_gain": max([item.get("delta", 0) for item in house_results] or [0]),
        "largest_loss": min([item.get("delta", 0) for item in house_results] or [0]),
        "revealed_at": time.time(),
        "narrator": "La Copa de las Casas ha escuchado sus apuestas. Algunas fueron estrategia; otras, contenido para terapia.",
    }
